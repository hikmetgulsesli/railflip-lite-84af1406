...
// Screen: Game Settings - RailFlip Lite (Enhanced)
// 
// AGENT INSTRUCTIONS:
...
export type GameSettingsRailflipLiteEnhancedActionId = "easy-1" | "medium-2" | "hard-3" | "save-preferences-4" | "back-to-game-5" | "reset-to-defaults-6";

export interface GameSettingsRailflipLiteEnhancedProps {
  actions?: Partial<Record<GameSettingsRailflipLiteEnhancedActionId, () => void>>;

}

export function GameSettingsRailflipLiteEnhanced({ actions }: GameSettingsRailflipLiteEnhancedProps) {
  return (
    <>
      ...
      <main className="relative z-10 w-full max-w-[420px] mx-margin-mobile animate-slide-up opacity-0">
      {/* Glassmorphic Card */}
      <div className="bg-surface-container/70 backdrop-blur-3xl border border-white/10 rounded-[1.5rem] p-lg shadow-[0_0_50px_rgba(0,0,0,0.5),inset_0_0_20px_rgba(0,242,255,0.05)] overflow-hidden relative">
      {/* Top Edge Glow */}
      ...
      <button className="flex-1 relative z-10 py-sm font-button-text text-button-text text-on-surface-variant hover:text-white hover:bg-white/5 rounded-lg transition-colors text-center text-[13px] uppercase tracking-wider" type="button" data-action-id="easy-1" onClick={actions?.["easy-1"]}>
                              EASY
                          </button>
      ...
      <button className="flex-1 relative z-10 py-sm font-button-text text-button-text text-on-surface-variant hover:text-white hover:bg-white/5 rounded-lg transition-colors text-center text-[13px] uppercase tracking-wider" type="button" data-action-id="hard-3" onClick={actions?.["hard-3"]}>
                              HARD
                          </button>
      </div>
      </section>
      ...
      <div className="relative w-full h-[12px] bg-surface-container-highest rounded-full border border-white/5 shadow-inner flex items-center group">
      {/* Styled Track Fill */}
      <div className="absolute left-0 h-[4px] ml-1 w-[60%] bg-gradient-to-r from-secondary-container to-primary-container shadow-[0_0_10px_rgba(0,242,255,0.6)] group-hover:shadow-[0_0_15px_rgba(0,242,255,0.8)] transition-shadow rounded-full z-0 pointer-events-none"></div>
      {/* Styled Thumb (Lead Pixel) */}
      <div className="absolute left-[60%] w-4 h-4 bg-white rounded-full shadow-[0_0_10px_#ffffff] group-hover:scale-110 transition-transform -ml-2 z-10 pointer-events-none border-[3px] border-surface group-hover:border-primary-container"></div>
      ...
      {/* Input Help Panel */}
      <section className="mb-xl bg-surface-container-high/50 hover:bg-surface-container-high/80 transition-colors p-md rounded-xl border border-white/5 flex items-start gap-md">
      ...
      {/* Primary Action */}
      <button className="w-full py-3 bg-primary-container text-on-primary-fixed font-button-text text-button-text rounded-xl hover:scale-[1.02] transition-colors uppercase tracking-widest font-extrabold relative overflow-hidden group animate-pulse-glow" type="button" data-action-id="save-preferences-4" onClick={actions?.["save-preferences-4"]}>
